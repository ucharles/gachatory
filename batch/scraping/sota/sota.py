...
from scraping.capsule_toy_classes import CapsuleToyCl, SotaCapsuleToyCl
...
def get_new_items():
    ...
    while True:
        ...
        try:
            container = soup.find("ul", class_="productsList")
            items = container.find_all("li", class_="productsList_item")
        except Exception as e:
        ...
        for item in items:
            ...
            name = (
                element.text
                if (element := item.find("p", class_="text")) is not None
                else None
            )

            try:
                capsule_toy = SotaCapsuleToyCl(
                    name=name,
                    detail_url=detail,
                    img=img,
                )
